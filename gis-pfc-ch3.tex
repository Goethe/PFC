...
% El propósito de este capítulo es documentar el proceso de desarrollo del subsistema de control y presentación o, en resumidas cuentas, el software de control. El objetivo más ambicioso del software de control es implementar un modo gráfico capaz de representar la forma de una señal al estilo de un osciloscopio digital, pero además, acompañar el fragmento representado de señal con la representación de su espectro en frecuencia y, por supuesto, permitir al usuario supervisor controlar la sesión de adquisición. La mayoría de las rutinas programadas en el código que conforma el software de control están destinadas a cubrir estos tres aspectos.
% Para conseguir una funcionalidad como la que ofrecen los osciloscopios digitales el software de control debe reunir una serie de requisitos mínimos:
% 
% \begin{itemize}
% 	\item En la representación de señales, debe ser capaz de representar el aspecto actual (con un retraso invisible al ojo humano) de una señal. La representación debe mostrar la señal disparada con respecto a un determinado de umbral y filtrada con el propósito de eliminar la componente en continua.
% 	\item Debe, asimismo, permitir al usuario modificar aspectos de la representación. Fundamentalmente la escala de los valores representados y duración del fragmento de señal representado.
% \end{itemize}
% 
% Vale, lo tengo casi claro. La idea es decir que a parte de dar una imagen de lo que se quiere lograr, lo que facilita ubicar de forma muy precisa que funciones se tiene que implementar y como, como van a ser las rutinas que conforman el modo gráfico, también se da una imagen del producto final
% Antes de ponerme a programar decidí a que se iba a parecer el software. Quise que se pareciera a un osciloscopio digital, que representara la señal como lo hacen los osciloscopios (que eliminase la componente en continua, que pudiese disparar la señal,..) y que pudiese controlar determinados aspectos de la representación (como la escala con que se representan las señales y la duración de la ventana).
...
\subsubsection{Modo convencional}
