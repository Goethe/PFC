...
% \section{Consecuencias de emplear la kpci-3108}
% Estas son un poco las competencias que debería tener la aplicación de control, aunque yo sé que los objetivos que se esperaban de la que se ha desarrollado con el proyecto abarcan éstas y algunas otras, en realidad no tanto, en lugar de almacenar presentar los datos. Sólo que es importante decir con detalle como se quieren representar.
% La consecuencia directa de emplear una tarjeta que necesita de un \textsc{pc} para funcionar y que no lo hace de forma autónoma es que se necesita de un software para el control de dicha tarjeta. El software de control debe encargarse de las siguientes tareas:

% \begin{itemize}
% 	\item Controlar la sesión de adquisición. Iniciar y detener la sesión. Permitir al usuario modificar aspectos de la sesión como son los puertos empleados, la frecuencia de muestreo, la configuración de los modos de adquisición entre otros.
% 	\item Dar formato a las muestras obtenidas para que estas puedan ser procesadas, almacenadas, manejadas por el usuario.
% 	\item Recuperar las muestras desde los búffers del dispositivo y almacenarlas en la memoria secundaria del \textsc{pc} para que el usuario tenga acceso a ellas.
% \end{itemize}
...
Puesto que la aplicación de control es necesaria y lo es también el esfuerzo de realizarla se decide con anterioridad al proceso de creación diseñar el software de forma que pueda desarrollar competencias adicionales. Así, además de las tareas básicas el software de control se diseña para que pueda cumplir con los siguientes cometidos.

\begin{itemize}
	\item Mostrar en un visor el valor de las muestras resultantes de muestrear la señal a 4 muestras por segundo.
	\item En un modo de funcionamiento diferente, mostrar en el mismo visor, cada cuarto de segundo aproximadamente, la media aritmética del valor de las muestras que de tiempo a recoger en dicho periodo a una frecuencia de muestreo especificada por el usuario.
	\item Funcionando en un modo gráfico debe ser capaz de representar la señal y el espectro en frecuencia de la misma en dos espacios de coordenadas separados. A voluntad del usuario pueden ser habilitadas o deshabilitadas a voluntad del usuario cualquiera de estas dos representaciones o ambas.
\end{itemize}
...
El argumento \texttt{id} es un indicador de dispositivo hardware. Se trata de un argumento opcional para tarjetas de sonido con \texttt{id} 0. El argumento \texttt{adaptador} requiere el nombre del adaptador de dispositivo hardware. A continuación, en el \vref{tab:adaptors} se muestra una relación con los adaptadores de dispositivo cuyo uso es más frecuente y el nombre de adaptador que debe introducirse como argumento de la llamada a \texttt{analoginput}. Por conveniencia se ha añadido Keithley a esta lista.
