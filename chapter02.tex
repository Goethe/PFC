...
\begin{figure}[htbp]
	\begin{center}
		\subfigure[Conector etiquetado como \emph{analog}]{\includegraphics[height=.3\textheight, keepaspectratio=true, angle=90]{chapter02-02.pdf}}\qquad
		\subfigure[Conector etiquetado como \emph{digital}]{\includegraphics[height=.3\textheight, keepaspectratio=true, angle=90]{chapter02-03.pdf}}
	\end{center}
...
\begin{table}[p]
	...
	\begin{tabular}{>{\raggedleft}p{1cm} >{\scshape}c >{\arraybackslash}l}
		...
		\midrule
		1 & ip5 & \multirow{11}{0.6\textwidth}{Bits digitales de entrada multifunción. Pueden ser configurados por el usuario para que ejerzan la función de:\miniit{\item Base temporal para el contador/temporizador y/o entrada a \emph{gate} \\\item Reloj externo para conversiones A/D o D/A \\\item Disparador digital externo \\\item Entrada digital en el modo \emph{target-mode}}} \\
		2 & ip3 & \\
		3 & ip1 & \\
		\\\\\\\\\\\\\\
		\midrule
...
\begin{table}[p]
	...
	\begin{tabular}{>{\raggedleft}p{1cm} >{\scshape}c >{\arraybackslash}l}
		...
		\multicolumn{1}{c}{Terminal} & {\upshape Puerto asignado} & \multicolumn{1}{c}{Descripción} \\
		\midrule
		1 & {\upshape Bit 0} & \multirow{4}{0.6\textwidth}{Canal 0 de bits de entrada/salida de propósito general (En la \kpci{} los bits digitales se agrupan de en ocho en ocho en canales. Los canales de este tipo puede configurarse para que los bits que lo integran se comporten como todo salidas o todo entradas)} \\
		2 & {\upshape Bit 1} & \\
		3 & {\upshape Bit 2} & \\
		$\vdots$ & $\vdots$ & \\
		8 & {\upshape Bit 7} & \\
