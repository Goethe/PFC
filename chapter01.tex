...
% Como se ha explicado en la introducción a la memoria uno de los objetivos que ha perseguido este proyecto de fin de carrera es el de implementar un sistema de adquisición y procesado de señales con el que llevar a cabo la segunda parte del mismo, la experimentación en palmeras mediante \textsc{endus}. Esta parte del documento trata el desarrollo de un sistema como tal. El protagonismo que tiene la tarjeta de adquisición de señales en el sistema de adquisición justifica que se dedique un capítulo completo a dar una descripción técnica de este dispositivo.\par
% Para desarrollar un sistema de adquisición y procesado de señales es necesario en primer término un dispositivo que permita realizar la adquisición de la señal. Con tal propósito se dispuso al iniciar el proyecto de una \kpci{} fabricada por \emph{Keithley}. En parte es debido a que se tiene acceso a este dispositivo por lo que se opta por desarrollar un sistema de adquisición de señales propio. Para comprender las posibilidades que proporciona una tarjeta de estas características se necesita una descripción técnica de la misma. En esta descripción técnica se exponen a continuación las características técnicas de la tarjeta, después pasa a darse una descripción funcional de la misma y se concluye explicando las posibilidades de conectividad. Un ulterior apartado proporciona una descripción del proceso de desarrollo de una nueva interfaz para facilitar la experimentación con la tarjeta.
...
El protagonismo que se le ha dado a la tarjeta \kpci{} la hace merecedora de un capítulo entero de este libro en el que se va a exponer en clave técnica y de forma resumida cuales son las propiedades que la hacen elegible como la base en la que se sostiene todo el sistema de adquisición. La \kpci{}, disponer de ella, es una de las razones principales por las que se ha optado por desarrollar un sistema de adquisición propio, de propósito, además de la principal razón, como es obvio, contar con un sistema diseñado a medida. Resulta primordial conocer las características de una pieza tan importante del conjunto, de hecho es el primer paso que se toma en la ejecución del proyecto, estudiar el aparato y averiguar hasta dónde alcanzan sus posibilidades. Una vez hecho esto es posible diseñar el resto del dispositivo de medida.\par
% A modo de introducción un resumen del contenido que examina cada apartado de este capítulo.

% \begin{itemize}
% 	\item La primera sección recoge las cualidades que desde un punto de vista técnico posee el dispositivo. La importancia radica en que los experimentos se encuentran limitados por las posibilidades.
% 	\item Más adelante la descripción funcional muestra, sin profundizar en los detalles, como es el comportamiento interno de la tarjeta de adquisición.
% 	\item Finalizando el capítulo, en un último apartado se describen las conexiones con las que cuenta la tarjeta. En el curso del proyecto se desarrolla un accesorio para la tarjeta, una caja de conexiones, un punto incluido dentro de este apartado explica cuáles son los motivos que llevan a construir la nueva interfaz y resume la evolución del proceso de elaboración.
% \end{itemize}

% INCLUIR EN LA INTRODUCCIÓN AL APARTADO DE RENDIMIENTO SI PROCEDE/CONVIENE/OPORTUNO/\dots Atención: Debo decir que el rendimiento de la tarjeta limita la frecuencia de trabajo de los transductores que vayan a utilizarse con ella pues se necesita una frecuencia de muestreo unas cinco veces superior a la frecuencia de la señal muestreada para poder recuperar la forma de la onda (debe advertirse de que la señal muestreada puede no ser periódica).


\section{Características técnicas del hardware}\label{sec:technical-features}
...
Para la interacción con dispositivos externos, la \kpci{} dispone de dos conectores formato mini-\textsc{d} de 36 terminales que cumplen con el estándar \textsc{ieee} 1284 de protocolos de comunicación en paralelo. Si se observa la tarjeta como el rectángulo que forma desde un punto de vista geométrico y observándola de frente, los conectores quedan ubicados en un lado de la tarjeta contiguo a aquel en el que se sitúa la conexión \textsc{pci}. Todo ello de forma que tras el montaje del periférico en la placa base los conectores quedan expuestos en la parte posterior de la carcasa en la que providencialmente debe encontrarse instalada dicha placa, tal y como es habitual en este tipo de dispositivos.\par
En las figuras expuestas a continuación se etiqueta cada terminal según su ubicación relativa con respecto al conector y al resto de terminales en el mismo, para cada conector, mostrando la distribución definida por el fabricante. Las tablas describen el propósito de cada terminal y se especifica que tipo de señal debe circular por los mismos. % Las tablas y figuras dispuestas (disposición)

\newsavebox{\auxbox}
\sbox{\auxbox}{\includegraphics[height=.3\textheight, keepaspectratio=true, angle=90]{chapter01-02.pdf}}
\newlength{\auxheight}
...
\begin{figure}[htbp]
	...
	\subfloat[Conector etiquetado como ``analog'']{\vbox  to \auxheight{ %
		\hbox to \auxwidth{
			\hspace*{-7pt}\includegraphics[height=.3\textheight, keepaspectratio=true, angle=90]{chapter01-02.pdf}} %
	\vfil}}\qquad
	\subfloat[Conector etiquetado como ``digital'']{\vbox to \auxheight{\vfil %
		\hbox to \auxwidth{
			\hspace*{-7pt}\includegraphics[height=.3\textheight, keepaspectratio=true, angle=90]{chapter01-03.pdf}} %
	\vfil}}
...
Una vez recopilada esta información, se planea la construcción de una caja de conexiones. El diseño de la caja contempla que se equipe ésta con 72 conectores banana hembra soldados a los 72 hilos de cobre y que comunican con los terminales de los conectores mini-\textsc{d} en los que termina el otro extremo de los cables. Además se planifica la inclusión de cuatro conectores coaxiales con rosca con conexiones redundantes a las que se presupone son las cuatro puertas cuyo uso será más frecuente, las correspondientes al primer canal analógico diferencial y las tierras analógicas; para así facilitar el uso continuado de las mismas. Por último se decide etiquetar la caja de conexiones con marbetes que reproduzcan la nomenclatura visible en la \vref{fig:ports}.\par
...
Puede observarse una representación de la caja de conexiones ya terminada en la \vref{fig:connection-box}. Así mismo se ha añadido un diagrama que representa dicha caja con mayor claridad.
