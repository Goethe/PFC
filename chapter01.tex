...
% El protagonismo que se le ha dado a la tarjeta \kpci{} la hace merecedora de un capítulo entero de este libro en el que se va a exponer en clave técnica y de forma resumida cuales son las propiedades que la hacen elegible como la base en la que se sostiene todo el sistema de adquisición. La \kpci{}, disponer de ella, es una de las razones principales por las que se ha optado por desarrollar un sistema de adquisición propio, de propósito, además de la principal razón, como es obvio, contar con un sistema diseñado a medida. Resulta primordial conocer las características de una pieza tan importante del conjunto, de hecho es el primer paso que se toma en la ejecución del proyecto, estudiar el aparato y averiguar hasta dónde alcanzan sus posibilidades. La tarjeta es el fundamento a partir del cual se construye el resto del sistema de medida, parte de sus capacidades las hereda el sistema, sus limitaciones deben tenerse en cuenta en la etapa de diseño, de ahí la importancia de estudiar a conciencia su funcionamiento y propiedades.\par % Esto constituye la base para construir el resto del dispositivo/es la base/los fundamentos a partir de los cuales se construye el resto del sistema de adquisición. Una vez hecho esto es posible diseñar el resto del dispositivo de medida.
...
En la \ref{fig:ports} se etiqueta cada terminal según su ubicación relativa con respecto al conector y al resto de terminales en el mismo, para cada conector, mostrando la distribución definida por el fabricante. Las tablas describen el propósito de cada terminal y se especifica que tipo de señal debe circular por los mismos. % expuestas a continuación 
...
Puede observarse una representación de la caja de conexiones ya terminada en la \vref{fig:connection-box}. Así mismo se ha añadido un diagrama que representa dicha caja con mayor claridad en la \vref{fig:connection-diagram}.

\begin{figure}[htbp]
	\begin{center}
		\includegraphics{chapter01-05.pdf}
	\end{center}
	\caption{Diagrama que representa la caja de conexiones}
