...
proporcionar, o bien el valor típico del \sig{spl} a la frecuencia de
resonancia, o bien un perfil de la magnitud frente a la frecuencia de la
onda eléctrica. Con independencia de que el \sig{spl} se presente mediante
...
presión que generalmente se especifica también. Para calcular el \sig{spl}
...
transductores de gama baja en un \sig{endus} radica en que emiten una
...
\begin{equation}
	\begin{split}
		...
		\mid_{R_\text{in}\simeq\infty} =
		C_\text{L}\left(R_\text{L} + R_0|\!|(R_\text{a} +
		R_\text{b})\right)\mid_{R_0 \simeq 0} =
		C_\text{L}\cdot R_\text{L} \\
		&= 700\cdot2.2\cdot 10^{-9} = 1.54\mu s
	\end{split}
\end{equation}

Finalmente se observa que, con los valores de periodo, tensión en el estado
alto y ciclo de trabajo de la señal que proporciona el acondicionador
(cuadro), y considerando una constante de tiempos pequeña, la respuesta del
actuador es muy rápida y la señal en sus bornes sigue con bastante
fidelidad la señal que puede medirse con el circuito en abierto.
