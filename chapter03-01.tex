...
\begin{figure}[htbp]
	\begin{center}
		\includegraphics{chapter03-01.mps}
	\end{center}
...
\begin{figure}[htbp]
	\begin{center}
		\includegraphics[scale=1, keepaspectratio=true]{chapter03-04.mps}
	\end{center}
...
\begin{figure}[htbp]
	\begin{center}
		\includegraphics[scale=1, keepaspectratio=true]{chapter03-05.mps}
	\end{center}
...
El principio en el que se fundamentan estas técnicas es similar al que rige las técnicas de diversidad convencionales. A partir de varias señales incorreladas entre sí se construye una señal cuya \textsc{snr} es considerablemente mejor que la \textsc{snr} de cualquiera de las señales primitivas por separado. Las técnicas de \textsc{ssp}, a diferencia de las técnicas basadas en la diversidad, consiguen sólo una seudo"=diversidad en frecuencia a partir de una única traza de la señal que se obtiene tras un \textsc{endus}.\par
Con frecuencia se obtienen buenos resultados empleando estas técnicas lo que, sumado a la sencillez con la que puede implementarse un experimento fundamentado en las mismas, recordando que no es requerida una redundancia real basada en la utilización de varias señales, las ha convertido en las técnicas más populares en el ámbito de los \textsc{endus}. A consecuencia de la reputación con la que se han visto revestidas recientemente numerosas investigaciones se han llevado a cabo en las décadas de los ochenta y los noventa, así como a principios de siglo, con el fin de mejorar en la medida de lo posible los buenos resultados que ya de por sí proporcionan las técnicas de \textsc{ssp}.\par
El procedimiento (más detallado en el \ref{tab:ssp-features}) que por lo general sigue todo el conjunto de técnicas de \textsc{ssp} consiste en aplicar una transformación sobre la traza que comprende dos pasos:
...
\begin{table}
	...
	\begin{tabulary}{.95\textwidth}{>{$(}c<{)$}Lr@{\hspace{6pt}}L}
		...
		df & Separación entre filtros & 4. & Calcular la \textsc{ifft} de las trazas de banda estrecha \\
		pb & Ancho de banda de los filtros & 5. & Normalizar la amplitud de las trazas de banda estrecha \\
		h_f & Tipo de filtro empleado & 6. & Construir una única traza a partir de las trazas de banda estrecha normalizadas \\
		\bottomrule
	\end{tabulary}
	\caption{Particularidades que en general se aplican a todos los algoritmos de \textsc{ssp}}
	\label{tab:ssp-features}
\end{table}

\begin{figure}[htbp]
	\begin{center}
		\includegraphics{chapter03-07.pdf}
	\end{center}
	\caption{Esquema genérico que representa el espectro del banco de filtros frente al espectro del transductor}
	\label{fig:filter-specter}
\end{figure}

% He hecho un corte del fichero aquí porque el realzado de sintaxis de Vim no interpreta correctamente el código que configura la disposición de las columnas en el entorno tabulado etiquetado con <<tab:ssp-features>>, en consecuencia considera todo el texto posterior incluido en una ecuación matemática y no resalta la ortografía.
\input{chapter03-02}
